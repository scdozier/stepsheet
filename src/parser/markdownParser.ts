...
function extractText(node: Paragraph | Content): string {
  ...
  if (node.type === 'strong') {
    // Preserve bold text content
    return (node as Strong).children.map((child) => extractText(child as Content)).join('');
  }
  if (node.type === 'emphasis') {
    // Preserve italic text content
    return (node as Emphasis).children.map((child) => extractText(child as Content)).join('');
  }
  if (node.type === 'image') {
