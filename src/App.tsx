...
import {
  parseMarkdown,
  ParsedMarkdown,
  ChecklistItem as ParsedChecklistItem,
...
const updateItemById = (
  ...
  return items.map((item) => {
    ...
    if (item.children) {
      return {
        ...item,
        children: updateItemById(item.children, id, newCompleted),
      };
    }
    return item;
  });
...
const applyPersistedState = (data: ChecklistData, persistedTasks: TaskStateMap): ChecklistData => {
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    window.electronAPI?.onFileSelected(async (filePath: string) => {
      try {
  ...
  useEffect(() => {
    ...
    const activeId =
      currentItemIndex >= 0 && currentItemIndex < flatItems.length
        ? flatItems[currentItemIndex].id
        : null;
    setChecklistData((prev) => setActiveItem(prev, activeId, settings.highlightCurrentStep));
    // Note: checklistData is intentionally excluded from deps to avoid infinite loop
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  <button
                    ...
                    style={{
                      ...
                      fontWeight: 'bold',
                      backgroundColor:
                        settings.textScale <= TEXT_SCALE_MIN
                  ...
                  <button
                    ...
                    style={{
                      ...
                      fontWeight: 'bold',
                      backgroundColor:
                        settings.textScale >= TEXT_SCALE_MAX
                          ? theme.colors.borderLight
                          : theme.colors.buttonBg,
                      color:
                        settings.textScale >= TEXT_SCALE_MAX
                    ...
                    settings.fileLibrary.map((entry) => (
                      <div
                        ...
                        style={{
                          ...
                          borderRadius: '4px',
                          backgroundColor:
                            loadedFilePath === entry.path ? theme.colors.activeBg : 'transparent',
                        }}
                      >
                        <button
